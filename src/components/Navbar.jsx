...
function Navbar() {
  ...
  return (
    <div className="w-full z-50 lg:h-8 px-5 lg:px-20 py-5 text-white flex justify-between lg:items-center fixed bg-slate-950/90">
      <div className="logo">
        <svg
          width="72"
          height="30"
      ...
      <div className={`${!menu && "hidden"} lg:flex`}>
        <div className={`lg:flex lg:gap-8 grid lg:h-auto h-96`}>
          {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map(
            (items, index) => (
              <a
                className={`${
                  index === 4 && "lg:ml-32"
                } font-extrabold text-4xl lg:font-normal lg:text-lg tracking-tighter uppercase lg:normal-case`}
                key={index}
              >
                {items}
              </a>
      ...
      <div className="lg:hidden">
        <div
          onClick={OnMenuClick}
          className={`flex-col ${
            menu ? "-space-y-1" : "space-y-1"
          } items-center justify-center cursor-pointer ${
            menu ? "pt-5 pb-5" : "pt-2 pb-2"
          }`}
        >
          <div
            className={`h-1 w-10 bg-white transition duration-600 ease-out ${
              menu ? "origin-content -rotate-45" : "origin-content rotate-0"
            }`}
          ></div>
          <div
            className={`h-1 w-10 bg-white transition duration-600 ease-out ${
